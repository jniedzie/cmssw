...
int main() {
  ...
  check(result.valid && result.resolved && result.status==DerivativeAuditResult::passed &&
        result.calls==4 && calls==4 && result.attempts.size()==1 && result.chosenStep==1.6 &&
  ...
  // A cubic derivative resolves first at scale .125 after all coarser scales.
  calls=0;
  ...
  check(result.valid && result.chosenStep==.125 && result.attempts.size()==8 && result.calls==32 && calls==32,
  ...
  check(result.valid && result.chosenStep==.16 && result.attempts.size()==1 && result.calls==4 && calls==result.calls &&
