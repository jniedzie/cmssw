  ...
  for (const HepMC3::GenVertexPtr &vitr : evt->vertices()) {
    ...
    for (const HepMC3::GenParticlePtr &pitr : vitr->particles_out()) {
      ...
      if (debugMuonPrimaries_ && std::abs(pdg) == 13 && status == 1) {
        ...
        std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                  << " stage=inventory decision=seen id=" << pitr->id() << " pdgId=" << pdg
    ...
    if (!qvtx) {
      if (debugMuonPrimaries_) {
        for (const HepMC3::GenParticlePtr &pitr : vitr->particles_out()) {
          if (std::abs(pitr->pid()) == 13 && pitr->status() == 1)
            ++muonFirstFailure["vertex_not_selected"];
          if (std::abs(pitr->pid()) == 13 && pitr->status() == 1)
            std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                      << " stage=vertex-selection decision=rejected id=" << pitr->id()
    ...
    for (const HepMC3::GenParticlePtr &pitr : vitr->particles_out()) {
      ...
      if (fPDGFilter) {
        ...
        if ((!pdgFilterSel && isInTheList) || (pdgFilterSel && !isInTheList)) {
          ...
          if (isMuon)
            std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                      << " stage=pdg-filter decision=rejected id=" << pitr->id() << " pdgId=" << pdg
      ...
      if (1 == status && std::abs(zimpact) >= Z_hector && rimpact2 <= theDecRCut2) {
        ...
        if (1 == status && (std::abs(zimpact) < Z_hector || rimpact2 > theDecRCut2)) {
          ...
          if (fPCuts && (ptot < theMinPCut || ptot > theMaxPCut)) {
            ...
            if (isMuon)
              std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                        << " stage=momentum-cut decision=rejected id=" << pitr->id() << " p=" << ptot
          ...
          if (fPhiCuts) {
            ...
            if (phi < theMinPhiCut || phi > theMaxPhiCut) {
              ...
              if (isMuon)
                std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                          << " stage=phi-cut decision=rejected id=" << pitr->id() << " phi=" << phi
          ...
          if (fEtaCuts) {
            ...
            if ((zi >= Z_lmin) & (zi <= Z_lmax) & (xi * xi + yi * yi < theDecRCut2)) {
              ...
              if (isMuon)
                std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                          << " stage=eta-cut decision=rejected id=" << pitr->id() << " projected=(" << xi / CLHEP::mm
                          << "," << yi / CLHEP::mm << "," << zi / CLHEP::mm << ") mm reason=eta_fiducial_cut"
                          << std::endl;
              continue;
            }
          }
          ...
          if (fLumiFilter && !fLumiFilter->isGoodForLumiMonitor(ppointer)) {  // MK: this function is always true
            ...
            if (isMuon)
              std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                        << " stage=lumi-filter decision=rejected id=" << pitr->id() << " reason=lumi_monitor_filter"
                        << std::endl;
            continue;
          }
          toBeAdded = true;
      ...
      if (isMuon && !toBeAdded) {
        ...
        std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                  << " stage=primary-selection decision=rejected id=" << pitr->id()
      ...
      if (toBeAdded) {
        ...
        if (g4prim->GetG4code() != nullptr) {
          ...
          if (fPtransCut && 1 == status && 0.0 != charge && px * px + py * py < theMinPtCut2) {
            ...
            if (isMuon)
              std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                        << " stage=pt-cut decision=rejected id=" << pitr->id()
        ...
        if (isMuon) {
          ...
          std::cout << "[FixedTargetMuonDebug][HepMC3] event=" << g4evt->GetEventID()
                    << " stage=g4-primary decision=accepted id=" << pitr->id()
